...
#let ntree = 500 since error stagnates after 500
bag_model_2 = randomForest(training_data$Sig~., data = training_data, mtry =9, importance = TRUE, ntree=500)
...
for(i in 1:(ncol(training_data)-1)){
  rf_model_1 = randomForest(training_data$Sig~., data=training_data, mtry=i, importance=TRUE, ntree=500)
  ensemble_error[i] <- rf_model_1$err.rate[200]
}
plot(ensemble_error,type = "l", main="OOB errors", xlab = "Number of Predictors", ylab = "Error Rate")

#number of predictors chosen
rf_model_2 = randomForest(training_data$Sig~., data=training_data, mtry=11, importance=TRUE, ntree=500)
